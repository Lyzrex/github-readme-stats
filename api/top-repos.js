...
export default async function handler(req, res) {
  try {
    ...
    // 1) Repos des Users holen (authentifiziert)
    let page = 1;
    const repos = [];
    while (true) {
      const r = await gh(
    ...
    // Fallback: authentifizierte /user/repos filtern (falls owner Schutz hat)
    if (repos.length === 0) {
      const r2 = await gh(`/user/repos?per_page=100&page=1`, token);
      if (r2.status === 200) {
        const list = await r2.json();
        repos.push(
          ...list.filter(
            (r) =>
              r.owner && r.owner.login.toLowerCase() === owner.toLowerCase()
          )
        );
      }
    }
